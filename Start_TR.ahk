...
class MyGui {
	...
	__New()
	{
		...
		Gui, MyWindow:Add, Button, x15 y+5 w60 gStart_TR, Start TR
		Gui, MyWindow:Add, Button, x15 y+5 w60 gUpdate_Clicked, Update
		Gui, MyWindow:Add, Button, x15 y+5 w60 gFirstRun, Setup

		Gui, MyWindow:Add, Button, x15 y+5 w60 gExit_Clicked, Exit

		Gui, MyWindow:Add, Text, x15 y+5, Data Timestamp:
		Gui, MyWindow:Add, Text, x15 y+5 vLastUpdated w220, % LastUpdated
	...
	Show() {
		...
		WinGet, OutputVar , MinMax, Start TR
		if (OutputVar = -1) {
			return
		}
		nW := this.Width
		nH := this.Height
		Gui, MyWindow:Show, w%nW% h%nH%, Start TR
	}
